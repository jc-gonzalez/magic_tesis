...
The first thing that comes to our mind is to divide the whole curve
$\hat{S}_{\mathrm{cr}}(E,\Theta)$ vs. $E$ by the solid angle used when
generating the incoming cosmic rays, that is by
...
expressing explicitly this solid angle) is perfectly valid. The
...
\end{equation}
%
Grouping the expression marked with
{\raisebox{6pt}{$\chronopair{\quad}$}\thickspace, we can write this
  as:
%
\begin{equation}
  \label{eq:Scr}
...
angle]. If want to get out of there something with units [area], we
simply multiply and divide, inside the integral, by \emph{any} solid
angle
%
\begin{equation}
  \label{eq:Scr}
...
collection area calculated for gammas. But, as we said, this can be
always as small as we want, and therefore any comparison would be
simply useless.

One could then think that we should divide just by the solid angle
where the efficiency is non-zero. But since the efficiency at given
off-axis angle depends on the energy of the primary, the Zenith Angle
and the core distance, so will this solid angle do. In other words, we
can not use a single solid angle for all energies, Zenith Angles and
core distances.
